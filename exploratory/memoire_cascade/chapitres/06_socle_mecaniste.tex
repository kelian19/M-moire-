...
la NB colle aux comptes firme-année, le Poisson rate la queue. Sources : scripts~\texttt{47}
et~\texttt{93} (règles de sélection du seuil).}
\label{soc:fig:validation}
\end{figure}

\paragraph{Le seuil est-il un choix ou une règle ? La question mérite d'être posée à l'envers.}
...
Une assurance de rédaction ne vaudrait pas cette comparaison.
...
$248$. \textbf{Aucune limite nouvelle ne s'ajoute donc à l'inventaire} du
...
$90\,\%$. Le quantile qui porte le capital n'est donc backtestable sur \textbf{aucun} historique
...
sévérité tient sur la population que le modèle tire. \textbf{Un seul mécanisme est en défaut,
...
traitement que le taux de dépassement. \textbf{Sur la thèse, la formule à retenir est
...
est plus légère. Sources : scripts~\texttt{89}, \texttt{90} et~\texttt{91}.
\end{cle}
