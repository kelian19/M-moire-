...
\vspace{4pt}
\textbf{Lecture.} Les cinq premières lignes sont l'\textbf{ossature} : le signe et l'ordre
...
\noindent Sources de la table : scripts~\texttt{25} (réaction du capital au score de conformité), \texttt{29} (branchement), \texttt{47} et~\texttt{63} (calibration figée), \texttt{61} (points intérieurs du pavé, dont le nombre de tirages du knockout), \texttt{66} (invariance en $g$), \texttt{67} (paramètres posés), \texttt{74} (additivité des coûts), \texttt{08h} (variantes de noyau et logique two-phase du rejet de l'auto-excitation) et~\texttt{89} (backtest à origine glissante).
...
\subsection*{La posture retenue, et les trois raisons de ce choix}
\label{sec:posture-retenue}

\noindent Chiffres de cette sous-section : scripts~\texttt{46} (VaR prédictive et sa précision),
\texttt{51} (échelle des six postures et bruit de simulation de chacune) et~\texttt{63}
(intervalle publié de la calibration figée).
...
et ce n'est pas une coïncidence numérique mais la même quantité sous deux noms. \textbf{Reporter le
...
disposition du lecteur, il porte sur ce qu'on appelle \og le \fg{} capital, et c'est précisément
pour cela que la question se tranche par la définition réglementaire plutôt que par la prudence.

Le paragraphe qui précède donne la grille ; il faut maintenant trancher, sans quoi le mémoire
laisserait son résultat central sans convention de report. \textbf{La posture retenue est le
...
\textbf{La deuxième est une mesure, et c'est elle qui rend le choix confortable.} La prédictive,
...
bruit de simulation. \textbf{La bonne réponse ne déplace donc pas le point.} Reporter la
...
coïncidence n'est pas générale et il faut dire où elle cesse, l'écart passant à $+4\,\%$ à
...
Échelle de sévérité non stationnaire & sous-estime & dérive de $4{,}00\,\%$ par an dans la queue ($p=0{,}0386$) ; au moins $+24{,}7\,\%$ sur le quantile, borne basse (\S\ref{soc:sec:backtest}) \\
\midrule
...
\vspace{4pt}
\noindent La lecture est nette sur les six premières lignes. Les trois écarts
\emph{involontaires} d'estimation sous-estiment tous le capital ; les trois choix
\emph{délibérés} le majorent tous, et de beaucoup plus. Le mémoire ne se protège donc pas
derrière ses erreurs, il les déclare, et sa prudence vient d'ailleurs, de choix assumés qu'il
nomme comme tels. C'est aussi pourquoi corriger les trois premiers ne renverserait aucune
conclusion : ils restent dominés par les seconds.

\vspace{2pt}
\noindent \textbf{Un mot sur la troisième ligne, qui est nouvelle et d'une autre taille.} Les deux
...
variation de la seule \emph{échelle} de sévérité, où elle est d'ailleurs remarquablement exacte,
...
l'\textbf{invariance d'échelle}, qui vaut pour toute amplitude, et non l'invariance à la dérive,
qui ne vaut pas. Source : script~\texttt{90}.
...
niveau de $99{,}5\,\%$. \textbf{Le quantile qui porte le capital n'est donc backtestable sur aucun
...
$\xi$. Source : script~\texttt{91} (\S\ref{soc:sec:backtest}).
...
\texttt{63}, \texttt{65} (expositions et besoins du panel), \texttt{74}, \texttt{78},
\texttt{89} (backtest hors échantillon et dérive d'échelle), \texttt{90} (effet de la dérive
sur l'écart entre états) et~\texttt{91} (charge agrégée et puissance du backtest).
