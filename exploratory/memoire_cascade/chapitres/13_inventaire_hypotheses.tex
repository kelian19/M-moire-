...
\noindent Sources de la table : scripts~\texttt{25} (réaction du capital au score de conformité), \texttt{29} (branchement), \texttt{47} et~\texttt{63} (calibration figée), \texttt{66} (invariance en $g$), \texttt{67} (paramètres posés) et~\texttt{08h} (variantes de noyau et logique two-phase du rejet de l'auto-excitation).
...
\paragraph{Le motif n'est pas que cet étage serait petit : c'est le plus grand des trois.} Il
faut le dire dans cet ordre, parce que l'argument inverse serait malhonnête. Sur son axe
dominant, la famille de queue, il vaut un facteur $5{,}5$, contre $2{,}5$ pour le paramètre et
une bande de $6\,858$ à $8\,697$~M\euro{} pour l'identification. Le retirer de l'intervalle
\textbf{rétrécit donc l'affichage plus que ne le ferait aucun autre retrait}, alors que
l'incertitude sous-jacente ne bouge pas d'un euro. C'est exactement le mouvement contre lequel
il faut se garder, et c'est pourquoi il ne s'agit pas d'une suppression mais d'un
\textbf{déplacement}.

\paragraph{Ce qui justifie le déplacement.} Les deux premiers étages sont de nature
\emph{statistique} : ils mesurent ce que la donnée disponible ne permet pas de trancher, à
modèle fixé, et un intervalle est l'objet qui convient pour les porter. Le troisième est de
nature \emph{épistémique} : il mesure l'effet d'un choix de famille, et un choix ne se moyenne
pas. Un intervalle qui cumule les trois mélange une incertitude d'estimation avec une
divergence de spécification, et il devient illisible : le lecteur ne sait plus si la borne
haute est un scénario défavorable ou une autre théorie du risque. En le sortant, on gagne un
intervalle interprétable \emph{et} un axe de sensibilité qui se lit pour lui-même.
