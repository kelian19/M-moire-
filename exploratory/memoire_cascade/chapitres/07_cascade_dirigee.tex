...
\noindent Source : script~\texttt{03}, pour la part dirigée du classeur d'expert.
...
\[
  \boxed{\;W_{jk}\;=\;\text{propension du pilier } j \text{ (\textbf{source}) à entraîner la
  défaillance du pilier } k \text{ (\textbf{cible}).}\;}
\]
La \textbf{ligne} $j$ décrit donc ce que $j$ \emph{émet}, et la \textbf{colonne} $k$ ce que $k$
...
à la somme de ligne $s_j$, qui mesure l'\textbf{émission} du pilier $j$, et à la revendication
...
autres piliers de la même entité, pondéré par une matrice \textbf{asymétrique} $W$. En respectant
...
\noindent Source de cette section : script~\texttt{29}, qui imprime la normalisation
$\max_j s_j$ et les descendants directs par pilier.
...
\textbf{émet} vers les autres piliers :
\begin{equation}
\label{eq:norm}
...
\noindent Source de cette section : script~\texttt{03}, qui imprime le rayon spectral brut
de \textsc{trans}, le diviseur de Leontief et les parts obtenues.
...
seuils critiques ($\rho(W)=1$ à $g=1{,}78$ ; $R_0=1$ à $g=7{,}2$) sont hors domaine. (b) La
...
alarmiste} : même normalisée, elle approche son seuil critique \emph{quatre} fois plus vite que
la cascade réelle ($g^\star=1{,}78$ contre $7{,}2$). Une lecture naïve de TRANS crierait à
...
\noindent Sources de cette section : scripts~\texttt{03} (matrice de génération suivante,
$R_0$, seuils critiques) et~\texttt{29} (rayon spectral de $W$ et progéniture).
...
\noindent Source de cette section : script~\texttt{04}, qui balaie la barre de matérialité
et imprime le biais résiduel sur le capital.
...
\noindent Sources de cette section : scripts~\texttt{03} (corrélations et pentes de
récupération de $W$) et~\texttt{05} (faisabilité de la donnée).
...
\noindent Sources de cette section : scripts~\texttt{32} et~\texttt{61}, qui vérifient les
propriétés énoncées sur la matrice du classeur puis sur des matrices tirées au hasard.
...
sa vérification numérique, produite par le script \texttt{32\_proprietes\_formelles} : l'énoncé
...
sous-ensembles de taille au moins deux, \textbf{$22$ changent de criticité selon l'ordre}, avec
un écart atteignant \textbf{$6$ points sur une échelle de $10$}, soit $60\,\%$ de l'échelle, pour
...
\textbf{non-transitivité} de la dominance entre piliers, a été testée dans trois formulations :
la transitivité des corrélations qu'impose un modèle à facteur unique, la positivité de la
dépendance symétrisée, et l'existence de cycles de dominance par paire. \textbf{Aucune ne se
...
revendication est donc \textbf{retirée}. La dépendance à l'ordre exclut exactement les mêmes
...
de $10{,}6\,\%$ (amorce P5) à $19{,}9\,\%$ (amorce P1). \textbf{Cet écart n'est pas une
...
\paragraph{La direction n'est pas identifiée, et cela coûte (proposition~\ref{prop:nonid}).} La
co-occurrence observée est une fonction de la seule partie symétrique $S$, et le test
directionnel ne rejette pas l'hypothèse $A=0$ (chapitre~\ref{ch:identifiabilite} : asymétrie
observée $119$ contre un nul de permutation à $128 \pm 27$, soit $z=-0{,}33$, et aucune arête
significative sur $18$). Retourner la matrice déplace pourtant le capital de $3{,}6\,\%$ et la
perte moyenne de $7{,}9\,\%$ (les niveaux sont établis en partie~\ref{part:resultats} ; seuls les
écarts importent ici), et surtout \textbf{la priorité de remédiation bascule de P1 à P4}. Sur
l'ensemble admissible, l'écart entre une matrice et sa transposée atteint $1\,173$~M\euro{}.

\begin{cle}
\textbf{L'énoncé à retenir : observationnellement indistinguables, matériellement différentes.}
C'est la formulation la plus nette de la frontière d'identification, et elle ne repose sur
\emph{aucun} dire d'expert. Elle chiffre en euros de capital, et en inversion de priorité, le
prix de ne pas savoir dans quel sens la contagion circule. C'est ce qui fonde à la fois le
modèle en couches (chapitre~\ref{chap:partielle}) et la spécification de reporting adressée au
régulateur.
\end{cle}

% =====================================================================

\noindent Sources complémentaires de cette section : scripts~\texttt{40} (test de
réversibilité, loi nulle de permutation) et~\texttt{03} (valeurs du classeur).
