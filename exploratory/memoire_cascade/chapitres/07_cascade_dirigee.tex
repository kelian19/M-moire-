...
\paragraph{Un $\rho_{ij}$ par entité ne se calibre pas, et l'argument n'est pas le compte de
paramètres.} Un modèle peut porter plus de paramètres que d'observations et rester utile. Ce qui
tranche est l'information que la donnée porte sur une sensibilité, et elle se mesure en périodes
d'observation d'un panel entité $\times$ pilier. Une sensibilité n'apparaît d'abord jamais sur un
...
$813$ périodes ; sur une seule coupe transversale, aucun écart du domaine admissible n'en est
séparable, la statistique de test plafonnant à $0{,}47$ pour un seuil de $3{,}84$. Le même écart
...
tirage du facteur. Ce qui identifie une sensibilité est la répétition dans le temps et non le
nombre d'entités, comme l'asymétrie de $W$ ne s'identifie que par l'ordre temporel.

On retient donc le compromis hiérarchique $\rho_{ij}=\rho_j+u_{ij}$, où la sensibilité de chaque
...
l'emboîtement est propre. La figure~\ref{fig:reseau-W} donne la structure de $W$ retenue et
l'allure d'un choc qui s'y propage.
