...
alors possibles. On peut poser $W$ au jugement d'expert et publier un SCR
ponctuel : c'est simple, lisible, et entièrement suspendu à un jugement qui n'a pas encore
été élicité. Ou bien on peut refuser de choisir, caractériser l'ensemble des $W$
compatibles avec ce que l'on sait, et publier des bornes. Ce chapitre construit
...
contagion ($W = 0$), on obtient le SCR nu : $5\,275$~M\euro{} dans le calage de
référence. C'est le plancher défendable, et il ne repose sur aucun paramètre non
identifiable.

Cette expérience d'ablation, qui consiste à retirer la propagation dirigée pour voir ce que la
structure porte réellement, se retrouve à l'identique dans le préprint de cascade climatique cité
au chapitre~\ref{ch:positionnement} \citep{ccrn2026} : y annuler la propagation effondre à la fois le
point d'attachement et la mesure de queue de leur portefeuille. La convergence n'est pas
anecdotique, car c'est le seul protocole qui sépare proprement ce qu'un modèle de cascade
\emph{apporte} de ce qu'il \emph{hérite} de ses marges.
...
\]
Les statistiques de co-occurrence que fournissent les sources identifient $S$ au premier ordre ;
aucune n'identifie la direction, donc $A$ (chapitre~\ref{ch:identifiabilite} et
proposition~\ref{prop:nonid}). L'ensemble admissible est alors
\[
...
Les bornes sont obtenues par énumération exhaustive des $2^{10} = 1\,024$ sommets
du pavé, ce qui les rend déterministes et reproductibles, au prix d'une hypothèse qu'il faut
énoncer, et que l'on vérifie plus bas. À $t=1$ :
\[
  \mathrm{SCR} \;\in\; [\,6\,858 \;;\; 8\,697\,]~\text{M\euro{}} ,
\]
soit un facteur $1{,}27$ entre les deux bouts. Rapporté au socle, cela s'énonce ainsi :
la contagion ajoute entre $+30\,\%$ et $+65\,\%$ au capital, et la donnée ne
permet pas de trancher plus finement. Le point d'expert ($8\,110$~M\euro{}) est un point
de cet ensemble, jamais sa mesure.

\begin{attention}
\textbf{L'hypothèse derrière l'énumération, et sa vérification.} Énumérer les sommets ne suffit
à borner que si les extrêmes y sont \emph{atteints}. L'argument usuel, selon lequel les extrêmes d'une monotone coordonnée par coordonnée vivent aux sommets, ne s'applique pas
directement ici : augmenter $A_{jk}$ élève $W_{jk}$ mais \emph{abaisse} $W_{kj}$, de sorte que
la réponse n'est pas monotone en $A_{jk}$ mais résulte d'un arbitrage. Rien ne garantit donc
\emph{a priori} l'atteinte aux sommets, et si elle était fausse les bornes publiées seraient trop
étroites. On procède alors par réfutation : $3\,000$ points intérieurs du pavé
admissible, tirés uniformément, sont évalués, et aucun ne sort des bornes ci-dessus, ni
en VaR ni en perte moyenne. L'hypothèse n'est pas réfutée, et les bornes sont défendables, mais
elles sont énoncées ici comme une hypothèse vérifiée numériquement, non comme un
théorème. La distinction est mince~; la passer sous silence ne le serait pas.
\end{attention}
...
sensibilité plat et trompeusement rassurant. La contribution se lit sur la perte
moyenne (bornes $[849\,;1\,100]$~M\euro{}, estimateur bien moins bruité) et surtout sur
la décision, c'est-à-dire l'ordre de remédiation.
\end{attention}
...
tomber la largeur des bornes de $1\,839$ à $695$~M\euro{}, soit $-62\,\%$. La
...
survit qu'à une élicitation réussie ne protège de rien.
...
sans que rien dans le régime~A ne signale ce déplacement au lecteur. La bande du
...
résultat acquis sans elle. Sur l'ensemble admissible, P5 n'arrive jamais en
tête, P3 dans moins de $10\,\%$ des cas, et P1 ($45\,\%$) et P4 ($36\,\%$) concentrent
$81\,\%$ des configurations. L'élicitation sert donc à départager P1 et P4 et à
...
\paragraph{Lecture.} Le niveau bouge d'un facteur $41$. Il est fixé par la
...
\emph{rapports} varient d'au plus $33\,\%$ de leur valeur moyenne, soit un ordre de
grandeur trente fois plus faible, et le groupe de tête de la priorité de
remédiation est identique dans les trois configurations.
% SOURCES-SCRIPTS: 33

\begin{attention}
\textbf{Où la stabilité du relatif s'arrête.} Le rapport le moins
...
\emph{illustratif} et ne doit pas être lu comme une mesure. Ce que le modèle détermine,
...
\begin{figure}[htbp]
  ...
  \caption{Identification partielle de la contagion dirigée : le
résultat central du mémoire. (a)~Le coût de la donnée manquante, les bornes se
resserrant à mesure que l'ignorance directionnelle $t$ diminue ; c'est cette pente qui chiffre
en euros ce qu'un registre mieux spécifié apporterait. (b)~La lecture centrale du prior
d'expert, à l'intérieur de bornes qui, elles, n'en dépendent pas, de sorte que le résultat reste
opposable sans élicitation. (c)~La priorité de remédiation robuste, au sens du regret
maximal. (d)~La dominance majoritaire entre piliers sur l'ensemble admissible : P5
n'arrive jamais en tête, et P1 et P4 concentrent $81\,\%$ des configurations.}
  \label{fig:partielle}
\end{figure}
...
La direction peut se reparamétrer par l'excès propre à la source, $u_{ij}$, et se descendre en
crans, de $u$ libre au cran nul où aucune source ne se distingue ; un rang $1$ capte $74\,\%$ de la
direction posée et resserre les bornes de $25\,\%$ au prix d'une hypothèse de structure
\matcomp.
...
que P1 et P4, les deux piliers de tête, deviennent indiscernables (gains de
remédiation $1\,051$ contre $1\,052$~M\euro{}, écart nul), alors qu'ils sont séparés de
$353$~M\euro{} dès que la direction du classeur est rétablie. La direction est donc
...
quantile, elle se lit ici, sur une décision qui bascule.
\end{cle}

% SOURCES-SCRIPTS: 34

\section*{Ce que ce chapitre transmet à la suite}

Les deux chapitres qui suivent font parler le modèle : la conformité y devient un état qui
évolue (chapitre~\ref{chap:multietats}) et l'on en tire une trajectoire de capital, un
surcoût et un ordre de remédiation (chapitre~\ref{chap:resultats}). Ils ne relâchent pas la
discipline établie ici. Tout ce qu'ils calculent l'est \emph{à contagion fixée}, sur un
point de l'ensemble admissible : les bornes de ce chapitre enveloppent donc chaque niveau
qu'ils afficheront. On lira par conséquent leurs montants comme illustratifs, leurs écarts
comme robustes, et la priorité entre les deux piliers de tête comme ce que seule la direction,
non identifiée, peut trancher. Le reste du mémoire est l'application de ce principe, pas un
second modèle.
