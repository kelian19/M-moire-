...
\toprule
\textbf{Entité} & \textbf{Actifs} & \textbf{SCR publié} & \textbf{DORA} &
\textbf{Bande} & \textbf{DORA / SCR} \\
...
\textbf{CNP Assurances SA} & $\mathbf{309\,800}$ & $\mathbf{14\,800}$ & $\mathbf{294}$ &
...
\vspace{2pt}
La première lecture est celle qu'on attendait. Sur la plus grande entité du panel, la charge de
non-conformité vaut $294$~M\euro{} en point et $[233\,;316]$ en bande, soit $2{,}0\,\%$ de son
\textsc{scr} publié : un ordre de grandeur défendable, obtenu sans aucune donnée interne. Et
l'écart entre états de conformité subsiste à toutes les tailles, de $+41\,\%$ à $+47\,\%$, face
à une charge forfaitaire qui n'en distingue aucune. La thèse ne dépendait donc pas de l'entité
fictive.
...
\figcle{S22_entites_reelles.png}{Le SCR DORA d'entités réelles, à partir de leurs seuls chiffres
\textsc{sfcr}. \textbf{(a)}~les quatre entités lues sur la courbe d'élasticité du panel,
l'entité notionnelle du mémoire tombant au milieu d'elles. \textbf{(b)}~la charge et sa bande
d'identification face à la charge forfaitaire, en échelle logarithmique : le forfait croît avec
les provisions, la charge de contagion beaucoup moins vite, et c'est tout le sujet.
\textbf{(c)}~la charge rapportée au SCR publié révèle une borne inférieure de validité de la
transposition, que le mémoire publie plutôt que de la laisser découvrir. Source :
script~\texttt{65}.}{fig:entites-reelles}
