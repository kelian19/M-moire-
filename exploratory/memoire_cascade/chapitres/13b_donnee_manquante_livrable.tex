...
par complémentarité. Un registre d'incidents \textsc{tic} l'identifiant devrait :
...
premières exigences ci-dessus sont déjà des champs du schéma \textsc{veris}, standard ouvert de
...
cas et la seconde, sur au moins deux étapes, dans $1{,}19\,\%$ ; leur \textbf{intersection ne
...
\textbf{obligatoires deux champs déjà normalisés} dans un dispositif, la notification
...
\noindent Sources de cette section : scripts~\texttt{28}, \texttt{52} et~\texttt{08b}.

\noindent Sources de cette section : scripts~\texttt{52} et~\texttt{08b}.
...
\noindent Source de cette section : script~\texttt{66}.
...
\figcle{Z18_valeur_information.png}{La valeur de l'information, chiffrée et hiérarchisée : la
contribution réglementaire du travail. \textbf{(a)}~l'information rétrécit la bande de capital,
$66\,\%$ étant acquis par la seule lecture de sept rapports publics, tandis qu'une bande
d'amplitude subsisterait \emph{même les dix sens connus} : l'identification partielle est
structurelle, non provisoire. \textbf{(b)}~quelle dépendance documenter en priorité ; en bleu,
celles que le corpus couvre déjà. L'écart est considérable, de $28{,}9\,\%$ d'ambiguïté levée
pour la paire gouvernance-incidents à zéro pour tests-tiers, et la dépendance la plus précieuse
est précisément celle que le corpus ne couvre pas. Source :
script~\texttt{55}.}{fig:voi}
