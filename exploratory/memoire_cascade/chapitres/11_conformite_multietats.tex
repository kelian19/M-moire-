...
répond en réalité à une exigence de fond : \textbf{la propagation à l'intérieur d'un sinistre et
...
\noindent Source de cette section : script~\texttt{63}, qui imprime les paramètres gelés de la latente et de ses seuils.
...
\noindent Source de cette section : script~\texttt{67}, registre des multiplicateurs posés par état.
...
\og non calibré\fg{} depuis l'origine, ce qui est honnête mais insuffisant. Tant que le résultat est
énoncé comme \og le capital passe de $116$ à $169$~M\euro\fg{}, il hérite entièrement de ces trois
...
\subsection{La dépendance entre états : ce que le modèle porte déjà, et ce qu'il omet}
\label{sec:dependance-etats}

\noindent Chiffres de cette sous-section : scripts~\texttt{69} (invariance sur l'espérance),
\texttt{77} (quantile du mélange et son bruit) et~\texttt{94} (quantile de la loi des
configurations).

Une objection métier mérite d'être traitée explicitement, parce qu'elle est juste : le
...
Le modèle ne représente pas ce mécanisme, et il faut dire à la fois ce qu'il porte déjà et ce que
l'omission coûte.
...
Ce qui manque est sa \textbf{structure} : la dépendance est \emph{échangeable}, identique pour
...
fonction additive ne dépend que des \textbf{marges}, jamais de la dépendance : les termes croisés
...
figure sont au \S\ref{sec:ann-dependance-etats}.
...
\paragraph{Mais l'argument porte sur une espérance, et le capital est un quantile.} La réserve
mérite mieux qu'une phrase, car elle vise le c\oe ur de la démonstration. Ce qui vient d'être
établi concerne
\[
\mathbb{E}\bigl[\mathrm{VaR}\bigr] \;=\; \sum_{m} w_m \,\mathrm{VaR}\bigl(L \mid m\bigr),
\]
la \emph{moyenne des VaR conditionnelles} aux $32$ configurations. Un lecteur qui demande le
capital d'une entité dont l'état de conformité est incertain demande autre chose : le quantile de
la loi \textbf{mélangée} sur les configurations. Or la moyenne des quantiles n'est pas le quantile
du mélange, et surtout l'argument d'additivité ne s'étend pas au second, puisqu'il repose sur la
linéarité de l'espérance, dont un quantile ne dispose pas.

Les deux objets ont donc été calculés. Ils diffèrent de $904$~M\euro, soit $9{,}3\,\%$ :
$9\,739$~M\euro{} pour la moyenne des VaR conditionnelles, $10\,643$ pour le quantile du mélange.
L'écart est structurel et non fortuit, car à $99{,}5\,\%$ la queue du mélange est prélevée dans
les composantes les plus lourdes quand la moyenne pondère toutes les composantes, y compris les
conformes. \textbf{Le mémoire publie la première grandeur}, et il valait mieux le dire que le
laisser deviner.

\paragraph{Sur le quantile du mélange, l'invariance n'est pas démontrée, elle est seulement non
détectée.} Le balayage y déplace le capital de $334$~M\euro{} contre $2$ pour l'espérance. Mais
ce déplacement n'est pas monotone en $\delta$, et le bruit de tirage seul, mesuré sur huit graines
de mélange à $\delta = 0$, a une étendue de $344$~M\euro{} pour un écart-type de $94$. \textbf{Le
déplacement est donc du même ordre que le bruit et n'est pas résolu.} L'énoncé honnête est plus
faible que pour l'espérance : on ne peut pas affirmer que la structure déplace le quantile, on
peut seulement dire qu'on ne le voit pas à cette résolution. Dans les deux lectures le
déplacement reste petit devant l'incertitude déjà déclarée, ces $334$~M\euro{} valant $18\,\%$ de
la largeur de la bande d'identification. La conclusion de gestion ne change pas ; c'est le
\emph{statut} de la conclusion qui change, de démontré à non détecté, et cette nuance appartient
au lecteur.
...
visait autre chose, un quantile de la loi des \textbf{configurations} elle-même, c'est-à-dire de
...
de $+2{,}76\,\%$ et la moyenne de queue au-delà de $90\,\%$ de $+1{,}06\,\%$. \textbf{L'argument
...
Le mécanisme est la \textbf{polarisation}, et c'est la mesure la plus robuste de cet exercice
...
\textbf{saturation}, non une invariance. On ne demande pas à une loi à $32$ atomes la résolution
...
\noindent Source de cette section : script~\texttt{08b}, pour la forme du mélange.
...
\noindent Source de cette section : script~\texttt{63}.
