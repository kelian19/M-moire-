...
n'est pas identifiable sur les données publiques agrégées, quand sa co-occurrence
l'est. C'est un résultat d'identification, dont la théorie des processus réversibles donne
la raison de principe. Mais l'affaire ne s'arrête pas là : un corpus de rapports
...
\textbf{domaine de contrôle} défaillant, et une chronologie en quatre étapes
...
Le placebo établit un fait ; la théorie des processus réversibles en donne la raison,
...
pourquoi la donnée ne peut pas la voir. (a)~le courant $J_{jk}=\pi_jP_{jk}-\pi_kP_{kj}$
est non nul : le modèle porte bien une direction. (b)~l'énergie de fluctuation, c'est-à-dire
...
\textbf{P1 (gouvernance) n'est jamais une cible} ($9$ sorties, $0$ entrée) et P2
...
cet exemple est compatible avec les deux explications : une direction informatique
...
dominante de sa paire. (b)~le placebo directionnel est franchi ($z=+3{,}93$,
...
recouvrent encore $0{,}5$. Les douze liens faibles comprennent en revanche les cinq
sorties de $P_2$ et les quatre entrées de $P_1$ : le puits et la source pure sont,
...
présenter autrement. Ce qu'il reproduit est la structure, et elle seule. Enfin, en
...
D'abord, la comparaison la plus naturelle a un support vide. Opposer P1 $\to$ P2 $\to$ P3
...
Ensuite, le test de dépendance au prédécesseur n'a aucune puissance, et la cause est
...
Enfin, et c'est la précaution la plus importante, ce que les séquences apportent ne se
...
Un acquis positif subsiste, et il porte sur le modèle plutôt que sur la donnée : la chaîne
...
\textbf{frontière d'identification} à trois niveaux, une discipline sur l'usage de $W$
