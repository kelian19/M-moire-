...
\noindent Sources de cette section : scripts~\texttt{05} (effectifs des jeux candidats et
test de faisabilité) et~\texttt{57} (biais de taille et de troncature).

\noindent Sources complémentaires : scripts~\texttt{28} (mise en regard réglementaire),
\texttt{40} (test de réversibilité) et~\texttt{64} (loi nulle exacte et jackknife).

\noindent Sources complémentaires : scripts~\texttt{40} (test de réversibilité) et~\texttt{64} (loi nulle exacte et jackknife).
...
la mesure \textbf{physique} : $\mathcal{E}(f,f)$ est la forme de Dirichlet de la chaîne, c'est-à-dire
...
mesure n'y intervient. La confusion est facile et elle mérite d'être écartée explicitement,
parce que le même mot désigne, en tarification, un objet qui n'aurait ici aucun sens. Le
\S\ref{sec:ann-martingalisation} nomme la martingale, sa filtration et sa mesure, énumère les
hypothèses de la chaîne, distingue ce qui est \emph{testé} de ce qui est \emph{décomposé}, et
dit pourquoi aucun test de martingalité n'est possible sur un facteur latent.
...
\noindent Sources de cette section : scripts~\texttt{62}, pour les comptages de la fenêtre
d'événement, et~\texttt{35}, qui rejoue l'étude d'événement complète, différence de différences,
placebo et bootstrap compris. La sortie de ce dernier est désormais versionnée, de sorte que
toutes les quantités de cette section sont sous contrôle automatique.
...
notoires. Ce corpus \textbf{corrobore} donc la structure ; il ne la \emph{calibre} pas. Le
codage à juge unique est à ce titre une \textbf{limite déclarée de ce mémoire}, et non une étape
...
renforcerait l'énoncé sans le sauver : une mesure de \textbf{fiabilité inter-juges} par second
codeur en aveugle, encore \emph{en attente}, et une \textbf{relecture par praticiens} dont les
questions portent précisément sur l'ordre P1 vers P2, à partir de leur expérience de terrain et
non de rapports publics, désormais \textbf{engagée}.

\textbf{Et sa première réponse illustre la frontière au lieu de la lever, ce qui est le
résultat le plus utile qu'elle pouvait produire.} Une consultante de Nexialog, sollicitée sur
sept affirmations qu'il lui était demandé de \emph{contredire}, n'en a contredit aucune. Sur
celle qui oppose la contagion à la cause extérieure commune, elle donne son accord et
l'illustre : un défaut de recette avant mise en production entraîne davantage d'incidents. Or
cet exemple est \textbf{compatible avec les deux explications} : une direction informatique
sous-dotée teste mal \emph{et} gère mal ses incidents, sans que l'un ait causé l'autre. Le
jugement de terrain nomme donc un ordre qu'il ne distingue pas d'une cause commune, exactement
comme les trois sources de données du présent chapitre. C'est une corroboration de la
\emph{frontière}, non de la matrice, et c'est aussi la raison pour laquelle le mémoire écarte
le jugement d'expert comme instrument de calibration (annexe~\ref{ch:elicitation}).
\end{attention}
...
ne dit donc rien de l'\textbf{amplitude}. On lève les deux ensemble.

\paragraph{Le corpus passe à dix rapports, et le signal se renforce.} Trois incidents
s'ajoutent, tous documentés par une enquête publique : \textbf{CrowdStrike Falcon 2024}
(analyse technique de cause racine du 6 août 2024, où le validateur de contenu laisse passer un
fichier dont le nombre de champs ne correspond pas au modèle), \textbf{Log4Shell 2021--2022}
(premier rapport du \textsc{csrb}, où l'absence d'inventaire des dépendances logicielles
interdit de savoir où la bibliothèque était déployée) et \textbf{Raphaels Bank 2015} (amende
conjointe \textsc{fca}/\textsc{pra} pour manquement répété à la gouvernance de
...
\vspace{2pt}
Les deux dernières lignes comptent autant que la deuxième. Restreindre aux \textbf{sources
primaires} écarte l'hypothèse que le signal viendrait de la couverture de presse ; restreindre à
la \textbf{perspective entité} écarte celle qu'il viendrait du mélange des points de vue, une
...
ses transitions, l'ensemble des \textbf{piliers défaillants}, y compris ceux qui ont cédé sans
...
C'est la matrice \emph{ordonnée complète}, donc $S$ \textbf{et} $A$, là où la lecture par paire
ne donnait que le signe de $A$.

\begin{attention}
\textbf{Le résultat est asymétrique, et c'est lui qu'il faut retenir.} Sur les vingt liens
ordonnés, \textbf{douze} ont un crédible à $90\,\%$ entièrement \emph{sous} $0{,}5$, et
\textbf{aucun} entièrement au-dessus. Le corpus établit donc très bien \emph{où la contagion ne
...
recouvrent encore $0{,}5$. Les douze liens faibles comprennent en revanche les \textbf{cinq
sorties de $P_2$} et les \textbf{quatre entrées de $P_1$} : le puits et la source pure sont,
eux, établis en tant que tels.
\end{attention}

\begin{cle}
\textbf{Ce que l'extension confirme, et ce qu'elle ne déplace pas.} La corrélation entre la
matrice documentaire et le classeur d'expert n'est \textbf{pas} significative, ni sur la matrice
complète ($\rho=+0{,}08$) ni sur la seule partie antisymétrique ($\rho=+0{,}31$, $p=0{,}19$).
Le corpus ne reproduit donc pas le classement des coefficients, et il serait malhonnête de le
présenter autrement. Ce qu'il reproduit est la \textbf{structure}, et elle seule. Enfin, en
...
(de $0{,}27$ à $0{,}07$ en moyenne) tandis que la \textbf{part de direction}
$\lVert A\rVert/\lVert W\rVert$ ne bouge pas (elle reste à $60\,\%$). Le corpus établit un
\textbf{ordre, pas une magnitude}, et l'extension confirme la frontière à trois niveaux sur un
objet plus riche au lieu de la déplacer.
\end{cle}

\figcle{Z20_corpus_etendu_pij.png}{Le corpus étendu et la matrice ordonnée $p_{jk}$.
...
chiffrée du puits et de la source pure. \textbf{(b)}~les dix premiers liens ordonnés avec leur
crédible à $90\,\%$ : la direction ressort, l'amplitude reste large et aucun crédible ne passe
entièrement au-dessus de $0{,}5$. \textbf{(c)}~sensibilité à la sous-documentation des
défaillances non propagées : le niveau s'effondre, la part de direction tient. Source :
script~\texttt{59}.}{fig:corpus-etendu}
...
que les enquêteurs concluent ainsi. L'enjeu n'est pas mince, puisque \textbf{onze des vingt-deux
...
\paragraph{La loi nulle devient exacte, et c'est ce qui rend la mesure possible.} L'asymétrie se
...
indépendantes, la loi de $T$ s'obtient par \textbf{convolution exacte}, sans aucun tirage. On y
...
l'ampleur qu'il faudrait. Deux opérations, la seconde bien plus hostile : le \textbf{retrait},
où le rapport n'établissait rien, et le \textbf{retournement}, où l'arête allait en réalité dans
...
\textbf{inversé} quatre des onze arêtes, et dans l'agencement le plus défavorable, pour perdre
la significativité. La troisième est plus instructive encore : \textbf{aucun nombre de retraits
...
n'ait à la reconstituer contre nous.
\end{attention}
...
(la dégradation modifie $S$, donc l'ensemble admissible lui-même), le \textsc{scr} d'entité passe
de $169{,}0$ à $127{,}1$~M\euro{} quand $P_1$ devient complètement muette, soit $-24{,}8\,\%$.
Ce déplacement de $41{,}9$~M\euro{} vaut \textbf{$0{,}81$ fois la largeur de la bande
...
et il ne faut pas le présenter comme telle. Ce qui bouge, en revanche, est le \textbf{pilotage} :
...
$P_1$. Le second codage en aveugle du script~\texttt{54} n'est donc pas le préalable de ce
résultat : il en serait le \textbf{renforcement}. Ce que le mémoire publie ici est une
\textbf{borne adverse}, et le rapport de $0{,}81$ ci-dessus dit pourquoi elle porte : accorder au
...
Le dispositif est \textbf{préparé et disponible}, et c'est à ce titre qu'il est décrit ici. Le
kit couvre les \textbf{dix} récits du corpus étendu, chacun réduit aux faits établis par le
rapport, sans aucune de nos flèches, et les entités y sont anonymisées pour que le codeur ne
...
fichier de réponse lui-même. Sur les $10 \times 10 = 100$ cellules, notre codage de référence en
compte $22$ directionnelles, ce qui fixe la base d'accord sur laquelle le $\kappa$ de Cohen se
calcule. En l'absence de second jeu, le script \textbf{ne fabrique aucune donnée} et s'arrête :
un dispositif qui produirait un coefficient sans second codeur vaudrait moins que son absence.

\figcle{Z22_biais_narration.png}{Le biais de narration, mesuré au lieu d'être déclaré.
\textbf{(a)}~jackknife par incident : aucun rapport ne porte le signal à lui seul, les dix
retraits laissant $p \leq 2{,}4\cdot10^{-4}$. \textbf{(b)}~point de rupture, en $p$-valeur
...
l'asymétrie, elle l'inverse. \textbf{(c)}~le niveau de capital résiste, y compris avec $P_1$
rendue muette, et la bande d'identification se resserre à mesure que l'émission de $P_1$
disparaît. Source : script~\texttt{64}.}{fig:biais-narration}

\section{Les séquences ordonnées : une voie explorée et refermée}
\label{sec:sequences-ordonnees}

Une objection naturelle se présente ici, et elle est fondée. Les sections précédentes n'exploitent
du corpus que des \emph{paires} ordonnées, or des marges par paires ne déterminent pas une loi sur
les permutations : un triplet ordonné, du type P1 $\to$ P3 $\to$ P2, porte en général une
information qu'aucune paire ne porte. La voie mérite donc d'être suivie, et elle l'a été.

Les séquences se trouvaient déjà dans le corpus, implicites : chaque incident code un ensemble
d'arêtes dirigées, et composer les arêtes d'un même incident donne des chemins. Il en sort $9$
chemins de longueur deux pour $4$ séquences distinctes, et une chaîne de trois arêtes chez
Equifax. Le résultat est néanmoins négatif, de trois façons qu'il faut distinguer parce qu'elles
n'appellent pas la même suite.

D'abord, \textbf{la comparaison la plus naturelle a un support vide}. Opposer P1 $\to$ P2 $\to$ P3
à P2 $\to$ P3 $\to$ P1 exige que P2 émette, or P2 n'émet jamais, $0$ fois sur $22$ transitions, et
P1 n'est jamais atteint. Ce n'est pas un intervalle large, c'est un ensemble vide, et cela se
répond en corrigeant la question plutôt qu'en réclamant des données.

Ensuite, \textbf{le test de dépendance au prédécesseur n'a aucune puissance, et la cause est
structurelle}. Il exige d'un même pilier qu'il soit atteint, sans quoi il n'y a pas de prédécesseur
sur quoi conditionner, et qu'il ait au moins deux successeurs distincts, sans quoi la loi
conditionnelle égale la marginale par construction. Aucun pilier ne remplit les deux, et les deux
manques sont \emph{exclusifs} : P1 a trois successeurs mais n'est jamais atteint, P3, P4 et P5 sont
atteints mais n'ont qu'un successeur. Un corpus dix fois plus grand du même genre n'y changerait
rien ; le critère de réouverture porte sur sa structure avant son volume.

Enfin, et c'est la précaution la plus importante, \textbf{ce que les séquences apportent ne se
convertit pas en identification}. La clôture transitive des arêtes ordonne $8$ des $10$ paires de
piliers, et le graphe agrégé est acyclique. On serait tenté d'y lire une confirmation de l'ordre
partiel revendiqué plus haut : ce serait une faute. L'acyclicité s'obtiendrait deux fois sur trois
par hasard sur un graphe aussi peu contraint, et une clôture transitive n'ajoute aucune
observation indépendante, elle déduit des mêmes arêtes. La frontière d'identification de la
section suivante ne bouge donc pas d'un cran.

Un acquis positif subsiste, et il porte sur le modèle plutôt que sur la donnée : \textbf{la chaîne
des piliers n'est pas sans mémoire}, l'auto-évitement renormalisant la loi du successeur sur les
piliers non encore visités. Les triplets portent par conséquent une prédiction testable, gonflée
de $1{,}211$ en moyenne par rapport à une chaîne sans mémoire, et l'idée qu'ils seraient redondants
avec les paires est fausse. C'est un test à faire le jour où un corpus le permettra, et le
\S\ref{sec:ann-sequences} en donne le protocole et le volume requis, $158$ à $589$ chemins par un
même pilier.

\vspace{2pt}
\noindent Sources de cette section : script~\texttt{75}.
...
\noindent Sources de cette section : scripts~\texttt{55} et~\texttt{64}.
