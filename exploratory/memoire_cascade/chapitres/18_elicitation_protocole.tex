...
%--------------------------------------------------------------
% HARNAIS-HORS-SCRIPT: l'instrument lui-meme. Les seuls nombres y sont les trois niveaux de
% quantile demandes a l'expert, les bornes 0 et 1 d'une force de lien et les etiquettes P1 a P5
% des cinq piliers. Ce sont des proprietes de CONCEPTION du questionnaire, pas des resultats :
% aucun script ne les imprime et aucun ne le devrait.
...
la section précédente affirme, et le réutiliser si la décision était rouverte.

\begin{attention}
\textbf{La version reproduite est celle de neuf graines, et non celle de dix.} L'exemplaire
préparé en premier portait une dixième question d'étalonnage, sur l'indice de queue de sévérité.
Elle a été retirée pour les deux motifs exposés plus haut, et elle ne figure donc pas ci-dessous.
Un exemplaire à dix graines subsiste dans les documents de travail : il ne doit pas être soumis,
une graine dont la valeur vraie est mal posée inversant les poids au lieu d'ajouter du bruit.
\end{attention}
...
\subsection*{Partie A, les neuf questions d'étalonnage}

Des quantités mesurées dans le mémoire, dont la valeur vraie est connue de l'analyste et n'est
pas affichée au répondant. Elles servent à pondérer les experts, non à les piéger. Les trois
colonnes de droite sont vides dans les deux tables qui suivent : c'est l'espace de réponse du
formulaire, non une donnée manquante.

\begin{center}\small
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{@{}l p{8.2cm} c c c@{}}
\toprule
& \textbf{Quantité} & $\mathbf{5\,\%}$ & $\mathbf{50\,\%}$ & $\mathbf{95\,\%}$ \\
\midrule
A1 & Part des sinistres financiers portés par une cause commune (un tiers), en \% & & & \\
A2 & Délai médian entre la survenance d'un incident et sa déclaration, en jours & & & \\
A3 & Part des incidents déclarés plus d'un mois après leur survenance, en \% & & & \\
A4 & Durée médiane de confinement, fin de la brèche, en jours & & & \\
A5 & Nombre d'incidents TIC matériels par an pour une grande entité financière & & & \\
A6 & Délai médian de déclaration d'un incident de type piratage, en jours & & & \\
A7 & Indice de Gini de la concentration des sinistres sur les jours ($0$ régulier,
     $1$ très concentré) & & & \\
A8 & Nombre maximal de victimes d'une même cause commune en un jour & & & \\
A9 & Part des jours de l'année qui portent une cause commune, en \% & & & \\
\bottomrule
\end{tabular}
\end{center}
...
Force du lien dirigé de $k$ vers $j$, entre $0$ et $1$ : $0$ signifie qu'une défaillance du
pilier $k$ n'entraîne jamais celle de $j$, $1$ qu'elle l'entraîne quasi certainement dans la
...
\begin{tabular}{@{}l p{8.2cm} c c c@{}}
\toprule
& \textbf{Lien dirigé $k \rightarrow j$} & $\mathbf{5\,\%}$ & $\mathbf{50\,\%}$ &
$\mathbf{95\,\%}$ \\
\midrule
