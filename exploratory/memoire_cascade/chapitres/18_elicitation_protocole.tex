...
conjoncturel.} La direction de $W$ \textbf{n'est pas identifiable} sur les données disponibles,
et le chapitre~\ref{ch:identifiabilite} le démontre plutôt que de le déplorer : la grandeur que
la donnée observe est aveugle à la partie antisymétrique, donc une matrice et sa transposée y
sont indiscernables. Il en résulte que $W$ n'est pas calibrée au sens où le sont la fréquence et
...
Sur une structure ainsi posée, il ne reste qu'\textbf{un seul contrôle externe possible} : le
jugement de praticiens qui ont vu des défaillances réelles, et qui n'ont lu ni le corpus ni le
mémoire. C'est ce qui rend ce dispositif nécessaire, et non facultatif. Mais il faut voir ce
qu'il peut et ce qu'il ne peut pas : un praticien peut \textbf{corroborer ou contredire} une
structure, il ne peut pas la \textbf{calibrer}, faute de quoi on retomberait sur l'élicitation
...
sur lesquelles le modèle repose, en leur demandant de les \textbf{contredire}. L'instrument est
