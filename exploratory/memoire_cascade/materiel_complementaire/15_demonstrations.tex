...
% HARNAIS-HORS-SCRIPT: chapitre de demonstrations : les nombres y sont des constantes d'enonce (bornes, exposants, dates de theoremes), aucun n'est un resultat calcule par le pipeline.
\noindent Les preuves de stabilité de la cascade ($\rho(W)<1$ par normalisation de Leontief) et
de la propriété de branchement figurent à l'annexe~\ref{ann:cascade}, avec leur vérification
...
\section{Formes fermées du quantile de sévérité et de sa moyenne de queue sous GPD}
\label{soc:ann:varestvar}
Les objets de cette annexe portent sur la sévérité $X$ d'un \emph{sinistre unique}, non sur la charge annuelle agrégée : d'où les symboles $\qsev$ et $\qbarsev$ plutôt que $\VaR$ et $\TVaR$, réservés à la mesure de capital. La forme fermée du quantile découle directement de l'inversion de la fonction de survie POT $\bar F(x)=\zeta_u(1+\xi(x-u)/\sigma)^{-1/\xi}$. Pour la moyenne de queue, on utilise $\qbarsev_\alpha=\frac{1}{1-\alpha}\int_\alpha^1\qsev_s\,\dd s$ ; la stabilité de la GPD (l'excès au-delà de tout seuil $v>u$ reste GPD de forme $\xi$ et d'échelle $\sigma+\xi(v-u)$) donne $\E[X-\qsev_\alpha\mid X>\qsev_\alpha]=\frac{\sigma+\xi(\qsev_\alpha-u)}{1-\xi}$ pour $\xi<1$, d'où \eqref{soc:eq:tvar-gpd}. Pour $\xi\ge1$, $\int_u^\infty\bar F=\infty$ : la moyenne de queue diverge.
