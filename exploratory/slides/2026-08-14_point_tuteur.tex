...
% =====================================================================
% SLIDE AJOUTEE APRES LE CALL DU 7 AOUT : Hugo a demande de supprimer les conclusions tirees du
% bootstrap de l'indice de queue, l'intervalle etant trop large pour etre interprete. Le retrait
% a sa propre slide parce qu'il en dit plus qu'un retrait : la meme surinterpretation vivait
% ailleurs, et la slide portait en plus une valeur qu'aucun script ne reproduit.
\begin{frame}{Ce que j'ai retiré, sur ta demande}
\scriptsize
\begin{block}{L'intervalle sur l'indice de queue ne porte plus de conclusion}
$[0{,}30\,;0{,}83]$ est trop large pour être interprété : les lectures que j'en tirais sur la
slide de robustesse sont \textbf{supprimées}, et l'intervalle ne reste affiché que comme un
\emph{fait}. La limite déclarée du mémoire, elle, ne bouge pas.
\end{block}
\vspace{-1mm}
\begin{block}{Deux choses trouvées en faisant ce retrait, et la seconde est pire}
\textbf{1.} La slide portait $[0{,}32\,;0{,}84]$, valeur qu'\textbf{aucun script ne reproduit} :
le script imprime $[0{,}320\,;0{,}871]$ par delta-méthode et $[0{,}304\,;0{,}831]$ par bootstrap,
et la slide mélangeait la borne basse du premier avec une borne haute qui n'appartient à
aucun.\\[3pt]
\textbf{2.} La même surinterprétation vivait au \textbf{chapitre socle}, qui écrivait que l'indice
\og{}se stabilise autour de $0{,}6$\fg{} quand le balayage le fait descendre à $0{,}38$ au
percentile $95$. Le script imprimait pourtant, en clair, l'interdiction de l'écrire : je ne
l'avais pas lue.
\end{block}
\vspace{-1mm}
{\scriptsize Ce qui reste vrai et n'a pas bougé : la \textbf{largeur} est le vrai sujet, pas le
point. C'est un résultat sur la \emph{bande}, qui ne demande pas d'interpréter les bornes.
...
% =====================================================================
% SLIDE AJOUTEE APRES LE CALL DU 7 AOUT : demande de la table complete des leviers et de la
% reconciliation avec le Delta global, puis de l'analyse des effets croises. Le residu de
% +5 001 M etait imprime sans etre decompose ; il l'est desormais (script 68), et la table des
% leviers gagne deux colonnes que le deck precedent n'avait pas.
...
% TITRE CORRIGE. Il annoncait « la suite des 5 001 M d'interaction », ce qui invitait a lire
% l'attribution par PILIER comme une decomposition de l'interaction entre CANAUX. Ce sont deux
% objets distincts : +5 001 M entre les quatre canaux (script 68), +1 395 M entre les cinq
% piliers (scripts 20 et 20b). Ils ne se somment pas, et le second n'a meme pas un signe resolu.
...
% SLIDE AJOUTEE APRES LE CALL DU 7 AOUT : les mots « portage » et « plancher » n'ont pas ete
% compris, et la relecture montre qu'ils etaient effectivement mal employes. Le memoire ecrivait
% que le RETOUR (une duree) etait un plancher, alors que ce qui est minore est le BENEFICE :
% minorer le benefice MAJORE la duree. La borne etait annoncee a l'envers, dans le memoire comme
% sur la figure J6, et les deux sont corriges.
...
%   104, 127, 70 : comptes de pages et recommandation de l'Institut, metadonnees du document.
% A relire si ce compte de pages bouge : le corps a gagne deux pages le 10 aout avec la table
% des postures et les deux limites declarees au chapitre 13. Le 12 aout, le total est passe de
% 126 a 127 avec la table complete des canaux et la reecriture du portage : la page ajoutee est
% dans l'ANNEXE C, le corps restant a 104. Compte relu sur le PDF (PyMuPDF), pas sur un index.
...
\item Vérifier le \textbf{format} : $104$ pages de corps et $127$ au total, contre les $\sim70$
