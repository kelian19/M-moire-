...
est défendable (la sensibilité, §\ref{sec:sensi}, montre que les décimales exactes
importent peu).
...
$\text{TRANS}[a][b]$ & Force dirigée «~$a$ entraîne $b$~», \textbf{asymétrique}.
...
\subsection*{Colonne « Probabilité conceptuelle » (rappel, préalable)}
...
\subsection*{Colonne « Gravité conceptuelle »}
...
\text{cohérence} &= \operatorname{moyenne}\big(\text{TRANS}[\text{maillons}]\big) \in [0,1],
\qquad \text{amplification} = 0{,}5 + 0{,}8\times\text{cohérence} \in [0{,}5,\,1{,}3] \\[2pt]
...
\subsection*{Colonne « Criticité » = moyenne géométrique}
...
base, aucun scénario n'atteint «~Extrême~».
\end{cle}
...
\item \textbf{Seule la hiérarchie est revendiquée.} Le résultat défendable n'est pas
  «~\chaine{1\fl2} vaut 8~» mais «~un ordre causalement cohérent est plus critique que son
  inverse~» — vrai tant que TRANS est asymétrique, quelles que soient les décimales.
\item \textbf{Les paramètres de forme ($0{,}40$, $0{,}5$, $0{,}8$) sont les moins ancrés}
  — purement des choix de modélisation — mais la §\ref{sec:sensi} montre qu'ils ne portent
  pas la conclusion.
\end{enumerate}
...
On ne nie pas que «~tout dépend des valeurs~» : on le \textbf{teste}. On perturbe tous les
...
Plafond «~jamais Extrême~» & \textcolor{accent}{\textbf{fragile}} & franchi dans $55\%$ des tirages $\Rightarrow$ propriété du calage, à ne pas sur-vendre \\
\bottomrule
\end{tabular}
...
2{,}50\ (\text{sans écart ROOT})\;;\quad 2{,}30\ (\text{sans écart GBASE})\;;\quad
