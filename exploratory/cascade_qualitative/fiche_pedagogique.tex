...
\item \textbf{ROOT} instancie l'\emph{événement initiateur} de l'analyse probabiliste de
  risque et le \emph{nœud-graine} des modèles de cascade, dont l'influence se mesure par la
  taille attendue de la propagation~\cite{KaplanGarrick1981,Kempe2003}.
\item \textbf{TRANS} est une \emph{matrice de transition dirigée}, au sens des chaînes de
  Markov, des réseaux bayésiens~\cite{Pearl1988}, des probabilités d'activation d'arête des
  modèles de cascade~\cite{Kempe2003}, et des matrices d'influence du risque
  systémique~\cite{Battiston2012}.
...
\]
La moyenne \emph{géométrique} (et non arithmétique) \textbf{pénalise le déséquilibre}~\cite{Grabisch2009} :
...
\textbf{ROOT} = événement initiateur / nœud-graine & \S1 & Kaplan \& Garrick~(1981)~\cite{KaplanGarrick1981} ; Kempe et coll.~(2003)~\cite{Kempe2003} \\
