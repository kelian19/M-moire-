...
\smallskip
\noindent\textcolor{accent}{\textbf{Mise à jour du 23 septembre 2026, à sept jours du dépôt.}}
Trois ajouts et une correction. Une section \textbf{« Pourquoi ainsi, et pas autrement »} en fin de
partie~X, qui défend les \emph{décisions} et non plus seulement les résultats~: le gel de la
calibration, le défaut de calibration non corrigé, le choix de borner plutôt qu'estimer, le seuil,
l'écart entre le moteur et la définition, la source non recalculable, l'élicitation sans réponse,
l'anonymisation, l'exposition des limites, et ce que je ferais avec six mois de plus. Une annexe
\textbf{« le modèle en formules »}, qui pose la chaîne entière du sinistre au capital avec le
\emph{statut} de chaque bloc. Et \textcolor{accent}{les cinquante renvois de page ont été recalés
sur le PDF déposé courant}~: ils visaient une version à $204$~pages et étaient tous faux de trois à
huit pages. \textcolor{accent}{Le $\kappa^\star$ du corps disait encore $76\,\%$ quand la mise à
jour ci-dessous annonçait $78$~; corrigé.}

\smallskip
\noindent\textcolor{accent}{\textbf{Mise à jour du 21 septembre 2026.}}
...
renvoient au PDF déposé, $198$~pages.
...
connu d'avance.}
\appui{résumé p.~3--4, chapitre~9 p.~104--108.}
\end{question}
...
\appui{chapitre~10 p.~109--128, chapitre~2 p.~36--37.}
\end{question}
...
\appui{chapitre~9 p.~104--108, chapitre~10 p.~109--128.}
\end{question}
...
\appui{partie~IV de ce document, chapitre~2 p.~36--37.}
\end{question}
...
\appui{chapitre~7 p.~85--91, chapitre~6 p.~75--84.}
\end{question}
...
cette confusion.}
\appui{chapitre~4 p.~44--63, chapitre~5 p.~65--74.}
\end{question}
...
\appui{chapitre~5 p.~65--74, chapitre~11 p.~130--141.}
\end{question}
...
\appui{chapitre~7 p.~85--91, chapitre~10 p.~109--128.}
\end{question}
...
\appui{annexe~C p.~172--181, annexe~F p.~186--189.}
\end{question}
...
bande déjà publiée.}
\appui{chapitre~8 p.~92--102, chapitre~10 p.~109--128.}
\end{question}
...
\appui{chapitre~8 p.~92--102, annexe~F p.~186--189.}
\end{question}
...
\appui{chapitre~11 p.~130--141, annexe~F p.~186--189.}
\end{question}
...
\appui{chapitre~10 p.~109--128, chapitre~12 p.~142--144.}
\end{question}
...
\appui{résumé p.~3--4, annexe~G p.~190--191.}
\end{question}
...
\appui{annexe~G p.~190--191, conclusion p.~145--148.}
\end{question}
...
\appui{notice des annexes p.~154.}
\end{question}
...
\appui{notice p.~154, annexe~A p.~155--158.}
\end{question}
...
\section{Pourquoi ainsi, et pas autrement}

\noindent Les questions précédentes portent sur ce que dit le mémoire. Celles-ci portent sur les
\emph{décisions} qui l'ont fait, et c'est là qu'un jury mesure la maturité. Chacune a une réponse
en une phrase, et aucune ne consiste à s'excuser.

\begin{question}{\og Vous avez gelé votre calibration en cours de route. N'est-ce pas un aveu ?\fg{}}
\rep{C'est une décision de conduite de projet, et elle est datée. Au-delà d'un certain volume de
résultats publiés, un modèle qui bouge encore invalide \emph{en silence} ce qui est déjà écrit~:
des dizaines de nombres se déplacent sans qu'aucun déplacement soit attribuable à la correction,
et la piste d'audit est perdue. Le gel sépare la \textbf{correction d'erreur}, qui est restée
autorisée et pratiquée, de la \textbf{recalibration}, qui ne l'était plus. Son prix est publié
plutôt que caché.}
\appui{chapitre~11 p.~130--141, chapitre~14 p.~150--152.}
\end{question}

\begin{question}{\og Vous savez que votre taux de dépassement est incohérent avec votre seuil, et vous ne le corrigez pas ?\fg{}}
\rep{L'écart vaut $2{,}4\,\%$ sur la VaR, soit un quarantième de l'intervalle à $90\,\%$ de cette
même VaR. Rejouer la chaîne réinjecterait un bruit de Monte-Carlo du même ordre~: on remplacerait
un écart connu et signé par un déplacement non attribuable. Et l'écart est \textbf{recalculé à
chaque import} du module de configuration, donc il ne peut pas se périmer en silence. Une limite
chiffrée vaut mieux devant un jury qu'un nombre corrigé sans trace.}
\appui{chapitre~11 p.~130--141.}
\end{question}

\begin{question}{\og Pourquoi borner, au lieu d'estimer ?\fg{}}
\rep{Parce que la direction n'est pas \emph{mal estimée}, elle n'est \textbf{pas identifiée}, et
les deux appellent des réponses opposées~: un paramètre mal estimé appelle davantage de données,
un paramètre non identifié appelle une autre approche. La borne a en outre une propriété qu'un
intervalle de confiance n'a pas~: sa largeur est le coût de l'ignorance, et il est \emph{connu
d'avance} au lieu d'être subi.}
\appui{chapitre~8 p.~92--102, chapitre~9 p.~104--108.}
\end{question}

\begin{question}{\og Pourquoi ce seuil-là pour la queue ? Il vous arrange.\fg{}}
\rep{C'est la question à laquelle je réponds le plus volontiers, parce que la mesure va contre
l'insinuation. La règle de stabilité retombe \emph{exactement} sur le seuil publié, qui est aussi
le mieux ajusté de tout le balayage. Et la règle la plus permissive donnerait un quantile
\textbf{supérieur de $16\,\%$}~: le seuil retenu n'est donc pas celui qui maximise le chiffre du
mémoire, c'est le contraire.}
\appui{chapitre~5 p.~65--74, chapitre~11 p.~130--141.}
\end{question}

\begin{question}{\og Votre moteur ne simule pas la cascade que votre définition pose. Pourquoi ?\fg{}}
\rep{C'est exact, et le mémoire l'écrit au lieu de le laisser trouver. Le moteur simule une marche
qui ne repasse pas par un pilier déjà touché~; la définition pose un processus de branchement. Les
deux ont la \emph{même} matrice moyenne et la même progéniture espérée, et l'écart entre elles est
\textbf{mesuré à $-4\,\%$ sur le capital}. Le choix ne porte donc pas le résultat, et il est
déclaré.}
\appui{chapitre~6 p.~75--84.}
\end{question}

\begin{question}{\og Vous citez une source que personne ne peut recalculer. Est-ce sérieux ?\fg{}}
\rep{Elle porte la \emph{structure} du risque, les parts par vecteur d'attaque, jamais son
\emph{niveau}. Son statut est déclaré dans un encadré~: consultée puis non conservée, donc non
reproductible par un script du dépôt. J'ai préféré assumer une citation datée plutôt que
fabriquer une reproductibilité que je n'ai pas. La retirer retirerait une décomposition, pas une
incertitude.}
\appui{chapitre~4 p.~44--63.}
\end{question}

\begin{question}{\og Pourquoi n'avez-vous pas d'avis d'experts sur cette matrice ?\fg{}}
\rep{Le protocole a été \textbf{préparé et diffusé}, et il est resté sans réponse~: aucun panel
n'a pu être constitué. Ce n'est donc pas un choix de ne pas essayer. Et le mémoire chiffre ce que
l'essai aurait coûté s'il avait mal tourné~: un panel biaisé déplace le capital de
$777$~M\euro{} \emph{sans qu'aucun signal ne l'indique}, quand l'approche par bornes reste
invariante. Une élicitation faiblement étalonnée échangerait une ignorance mesurée contre une
incertitude non déclarable.}
\appui{annexe~C p.~172--181.}
\end{question}

\begin{question}{\og Pourquoi anonymiser des entités dont les rapports sont publics ?\fg{}}
\rep{Par prudence et non par confidentialité, et la nuance compte~: le modèle n'utilise que le
bilan, et l'état de conformité y est \emph{supposé}. Nommer une entité reviendrait donc à lui
attribuer une non-conformité que rien n'observe. La table de correspondance existe et vit dans un
script, de sorte que le travail reste vérifiable par qui en a le dépôt.}
\appui{chapitre~10 p.~109--128, chapitre~14 p.~150--152.}
\end{question}
...
faute. Le mémoire a d'ailleurs \textbf{réfuté sa propre thèse d'origine}, celle qui lui donnait
son titre, et requalifié son chiffre d'entité en borne supérieure. Ce qui tient dans ce travail,
ce n'est pas l'assurance du niveau, c'est que tout ce qui est affirmé est soit démontré, soit
mesuré, soit déclaré posé.}
\appui{chapitre~11 p.~130--141, chapitre~13 p.~145--148.}
\end{question}

\begin{question}{\og Et si vous aviez six mois de plus, que feriez-vous ?\fg{}}
\rep{Deux choses de structure, et elles sont écrites dans le mémoire. D'abord remplacer
l'hypothèse d'additivité des coûts par un modèle de coût à \textbf{contrainte de capacité}~: le
sens de l'effet ne se déclare pas aujourd'hui parce que la mutualisation de la remédiation et la
saturation de la capacité tirent en sens contraire, et un exposant unique ne peut pas porter les
deux. Ensuite étendre la cascade \textbf{entre entités} par le prestataire partagé, où le graphe
est \emph{observé} et où l'extension n'hériterait donc pas du problème d'identification qui
commande tout ce travail.}
\appui{annexe~F p.~186--189.}
\end{question}

\laphrase{Je n'ai pas cherché à rendre le chiffre plus sûr qu'il n'est. J'ai cherché à rendre
exact ce que l'on peut en dire, et à chiffrer ce que l'on ne peut pas.}
...
\mathrm{SCR} \;=\; \mathrm{VaR}_{99{,}5\,\%}(L) \;-\; \mathbb{E}[L].
\]
...
cohabite-t-il avec un capital de plusieurs milliards~?
\[
\text{facteur d'agrégation} \;\gtrsim\; \lambda^{\xi}
\qquad
\small\textcolor{navy}{\text{borne basse, non approximation centrée}}\normalsize
\]
\lepiege{Le terme additif de la forme fermée est négatif, donc $\lambda^{\xi}$ \textbf{minore} le
facteur au lieu de l'approcher. Et le rapport entre capital et quantile unitaire \emph{change de
sens} avec l'échelle~: $12{,}3$ au secteur, $0{,}30$ à l'entité. Aucun rapport fixe ne relie les
deux objets, donc les comparer ne conclut jamais, dans un sens comme dans l'autre.}
