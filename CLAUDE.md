...
synthèse en tête et des figures LUCY, puis la réduction de l'annexe : **`main_ensae` à 223 pages
dont 172 avant les annexes** (annexes en 173, une page gagnée le 17 septembre par la
seconde réponse de relecture), en Times 12 et interligne 1,5 ; v1 à **172 pages dont 125 de
...
| 98 | **ce que la co-occurrence voit de la direction** (17 septembre 2026). La proposition 6.7 d'origine était FAUSSE : la co-occurrence ne dépend de W que par S au premier ordre et sous amorce uniforme ; la loi complète des ensembles touchés distingue W de W^T (fourche contre collisionneur). Lois exactes de la marche (moteur publié) ET de la cascade indépendante, contrôlées contre `scr_engine` à 1e-16. Sous W^T, part multi-piliers 62,31 % → 50,77 %. **Le moteur publié est la marche, pas la cascade de la définition 6.1** |
| 99 | **provenance du multiplicateur de fréquence** (17 septembre 2026) : λ NC = 21,56 × 2,4875 = 53,62, parts Hackmageddon × centres des fourchettes S2 de `src/frequency/negbin.py`, contrôle contre `canaux_conformite.LAM_NC`. Les parts Hackmageddon ENTRENT donc dans le moteur par ce multiplicateur. Aux bornes des fourchettes, λ NC de 41,7 à 65,6 |
...
| 106 | **une seule observation porte-t-elle la queue ?** (23 septembre 2026). Déterministe, il réajuste et ne simule pas. **Deux moitiés opposées** : le jackknife sur les 91 excès donne une étendue de 111,7 M€, soit 17,9 % de l'IC90 publié, et **aucun** des 91 retraits n'en sort le quantile ; mais retirer les **trois** plus grosses pertes le fait passer sous la borne basse, et cinq le divisent presque par deux (657,2 → 327,3 ; ξ de 0,60 à 0,22). À citer comme **condition de lecture**, pas comme fragilité : retirer les plus grosses pertes d'une queue lourde retire l'objet mesuré. **Arrêt dur** repris du script 95 : il refuse de tourner si le compte d'excès n'est pas exactement 91 ou si ξ s'écarte de plus de 5 % de la valeur gelée. Exige `SAS_OpRisk_Global_Data_June_2026.xlsx` |
